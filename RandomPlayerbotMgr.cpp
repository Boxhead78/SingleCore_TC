...
bool RandomPlayerbotMgr::ProcessBot(uint32 bot)
{
    ...
    if (!player)
     ...
     //Boxhead - Moved player resurrection above battleground resurrection
    Map* map = player->GetMap();
    if (player->isDead() && !player->InBattleground() && map && !map->Instanceable())
    {
...
void RandomPlayerbotMgr::RandomTeleport(Player* bot, vector<WorldLocation> &locs)
{
    ...
    if (locs.empty())
    {
        sLog->outMessage("playerbot", LOG_LEVEL_ERROR, "Cannot teleport bot %s - no locations available - locs.empty", bot->GetName().c_str());
        return;
    }

    for (int attemtps = 0; attemtps < 30; ++attemtps)
    {
        ...
        uint32 zoneId = map->GetZoneId(x, y, z);
        //Boxhead - Don't spawn horde in alliance territory and the other way around
        if (bot->getRace() == RACE_HUMAN || bot->getRace() == RACE_DWARF || bot->getRace() == RACE_NIGHTELF || bot->getRace() == RACE_GNOME || bot->getRace() == RACE_DRAENEI)
            if (zoneId == 14 || zoneId == 17 || zoneId == 85 || zoneId == 130 || zoneId == 215 || zoneId == 1497 || zoneId == 1637 || zoneId == 1638 || zoneId == 3430 || zoneId == 3433 || zoneId == 3487)//These are all horde starting areas/towns
                continue;

        if (bot->getRace() == RACE_ORC || bot->getRace() == RACE_TROLL || bot->getRace() == RACE_TAUREN || bot->getRace() == RACE_UNDEAD_PLAYER || bot->getRace() == RACE_BLOODELF)
            if (zoneId == 1 || zoneId == 12 || zoneId == 38 || zoneId == 40 || zoneId == 141 || zoneId == 148 || zoneId == 1519 || zoneId == 1537 || zoneId == 1657 || zoneId == 3524 || zoneId == 3525 || zoneId == 3557) //These are all alliance starting areas/towns
                continue;

        //Boxhead - Don't spawn them at gm Island
        zoneId = map->GetZoneId(x, y, z);
        if (zoneId == 876)
            continue;
    ...
    sLog->outMessage("playerbot", LOG_LEVEL_ERROR, "Cannot teleport bot %s - no locations available - gave up finding a location", bot->GetName().c_str());
}
...
void RandomPlayerbotMgr::RandomizeFirst(Player* bot)
{
	...
	{
		level = GetMasterLevel();
        //Boxhead - Don't do the exact same level for all bots
        int leveldiff = urand(1, 5);
        if (leveldiff > 3)
            level -= leveldiff;
        else
            level += leveldiff;
	...
	for (int attempt = 0; attempt < 10; ++attempt)
	{
        ...
        if (!takePlayerLevel) {
            ...
            if (bot->getClass() != CLASS_DEATH_KNIGHT) {
                randomlevel = urand(minLevel, maxLevel);
                level = randomlevel;
            }
            else if (maxLevel >= 58) {
                randomlevel = urand(58, maxLevel);
                level = randomlevel;
            }
            if (level<=0) {
                sLog->outMessage("playerbot", LOG_LEVEL_ERROR, "Bot level lower than 1. Setting it to 1...");
                level = 1;
            }

            if (urand(0, 100) < 100 * sPlayerbotAIConfig.randomBotMaxLevelChance && sPlayerbotAIConfig.randomBotMaxLevel == sWorld->getIntConfig(CONFIG_MAX_PLAYER_LEVEL)) {
                level = maxLevel;
            }
        }
